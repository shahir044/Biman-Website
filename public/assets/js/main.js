    ...
    $window.on('load', function () {
        ...
        $document.on('click', '.add-flight-btn', function () {
            ...
            if ($('.multi-flight-field').length < 4) {
                ...
                var newaddress = $('.multi-flight-field:last').clone().insertAfter('.multi-flight-field:last');
                ...
                var optionsDepartMultiCity3 = $("#depart3"); 
                optionsDepartMultiCity3.html('');
                var optionsArrivMultiCity3 = $("#arriv3");

                $.each(airportName, function () {
                    optionsDepartMultiCity2.append($("<option />").val(this.iata_code).text(this.airport_name + ", " + this.city + " (" + this.iata_code + ") "));
                            ...
                            $.each(airportName, function () {
                                if (this.iata_code == to_airport) {
                                    optionsArrivMultiCity2.append($("<option/>").val(this.iata_code)
                                        .text(this.airport_name + ", " + this.city + " (" + this.iata_code + ") ")
                                    );
                                }
                ...
                $.each(airportName, function () {
                    optionsDepartMultiCity3.append($("<option />").val(this.iata_code).text(this.airport_name + ", " + this.city + " (" + this.iata_code + ") "));
                    // options.append($("<option />").val(this.iata_code).text(this.city + " (" + this.iata_code + ") "));
                });
                $('#depart3').change(function () {

                    var value = $(this).val();
                    optionsArrivMultiCity3.html('');
                    // destinationOptions.append(new Option('Select Destination', '0'));
                    optionsArrivMultiCity3.append("<option disabled='disabled' SELECTED>,Select Destination</option>");
                    ...
                    $.each(sampledata, function () {
                        ...
                        if (value == from) {
                            ...
                            $.each(airportName, function () {
                                if (this.iata_code == to_airport) {
                                    optionsArrivMultiCity3.append($("<option/>").val(this.iata_code)
                                        .text(this.airport_name + ", " + this.city + " (" + this.iata_code + ") ")
                                    );
                                }
